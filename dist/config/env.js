...
const optionalUrl = (message) => z.preprocess((v) => {
    if (typeof v !== 'string')
        return v;
    const trimmed = v.trim();
    return trimmed === '' ? undefined : trimmed;
...
const envSchema = z
    ...
    .refine((e) => Boolean(e.DATABASE_URL) || Boolean(e.DB_HOST && e.DB_USER && e.DB_PASSWORD), {
    message: 'Set DATABASE_URL (Supabase → Project Settings → Database → Connection string), ' +
