...
export class PublicService {
     ...
     */
    static async getMapProjects(county, status) {
        const conditions = [`(p."PublicationStatus" = 'Published' OR p."IsPublished" = TRUE)`];
        const params = [];
      ...
      ${where}
      ORDER BY p."IsFeatured" DESC, p."CreatedAt" DESC`, params);
    }
    /**
    ...
    static async getProjectRoute(projectId) {
        return queryOne(`SELECT "RouteId", "RouteName", "GeometryType", "GeoJson"
       FROM "ProjectRoutes"
       WHERE "ProjectId" = @projectId
       ORDER BY "RouteId" ASC
       LIMIT 1`, [{ name: 'projectId', value: projectId }]);
    }
    /** All project routes, for map preloading. */
    static async getAllProjectRoutes() {
        return query(`SELECT
