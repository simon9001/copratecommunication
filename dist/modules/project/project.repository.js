...
export class ProjectRepository {
    ...
    static async findById(id) {
        return queryOne('SELECT * FROM Projects WHERE ProjectId = @id', [{ name: 'id', value: id }]);
    }
    static async findBySlug(slug) {
        return queryOne('SELECT * FROM Projects WHERE Slug = @slug', [{ name: 'slug', value: slug }]);
    }
    static async findByCode(code) {
        return queryOne('SELECT * FROM Projects WHERE ProjectCode = @code', [{ name: 'code', value: code }]);
    }
       ...
       WHERE ProjectId = @id`, [
        ...
        if (dto.county !== undefined || dto.subCounty !== undefined || dto.latitude !== undefined || dto.longitude !== undefined) {
           ...
           WHERE ProjectId = @id AND IsPrimaryLocation = 1`, [
                ...
                await execute(`INSERT INTO ProjectLocations (ProjectId, LocationName, County, SubCounty, Latitude, Longitude, IsPrimaryLocation)
