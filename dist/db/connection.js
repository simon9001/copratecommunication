...
export async function checkRlsAccess() {
  ...
  `);
    if (rows.length === 0)
        return null; // schema not applied yet
    const r = rows[0];
    const seesAllRows = r.bypassRls || (r.isOwner && !r.rlsForced) || !r.rlsEnabled;
    const counted = await pool.query('SELECT COUNT(*)::int AS n FROM "Projects"');
...
export async function warnIfRlsBlocksAccess() {
    try {
        const report = await checkRlsAccess();
        if (!report)
            return;
        if (report.seesAllRows) {
            logger.info(`[RLS] Connected as '${report.role}' — sees all rows ` +
                `(${report.bypassRls ? 'BYPASSRLS' : report.isOwner ? 'table owner' : 'RLS disabled'}). ` +
                `${report.visibleProjects} project(s) visible.`);
            return;
        }
        logger.error(`[RLS] Row Level Security is filtering this connection. Role '${report.role}' ` +
