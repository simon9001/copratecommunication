...
export const errorHandler = (err, c) => {
    ...
    if (err instanceof AppError) {
        ...
        return c.json(createErrorResponse(err.message, err.errorCode, err.details, requestId), err.statusCode);
    }
    ...
    if (err instanceof ZodError) {
        ...
        return c.json(createErrorResponse('Validation error: invalid request payload', ErrorCode.VALIDATION_ERROR, formattedDetails, requestId), 400);
    }
    ...
    if (typeof pgCode === 'string' && /^[0-9A-Z]{5}$/.test(pgCode)) {
        ...
        switch (pgCode) {
            case '23505': // unique_violation
                ...
                return c.json(createErrorResponse('That record already exists. Check for a duplicate code, slug, or name.', ErrorCode.CONFLICT, detail ? { detail } : null, requestId), 409);
            case '23503': // foreign_key_violation
                logger.warn(`[Database Conflict] Foreign key '${constraint}' [ReqID: ${requestId}]`);
                return c.json(createErrorResponse('That referenced record does not exist, or is still in use elsewhere.', ErrorCode.CONSTRAINT_VIOLATION, detail ? { detail } : null, requestId), 409);
            case '23502': // not_null_violation
            case '23514': // check_violation
                logger.warn(`[Database Constraint] '${constraint}' rejected the value [ReqID: ${requestId}]`);
                return c.json(createErrorResponse('A required value is missing or outside the allowed range.', ErrorCode.VALIDATION_ERROR, detail ? { detail } : null, requestId), 400);
            case '42P01': // undefined_table
                ...
                return c.json(createErrorResponse('The database schema is not initialised.', ErrorCode.DATABASE_ERROR, null, requestId), 500);
            default:
                ...
                return c.json(createErrorResponse('Database operation failed due to a server constraint or error', ErrorCode.DATABASE_ERROR, null, requestId), 500);
        }
    }
    ...
    return c.json(createErrorResponse('An unexpected internal server error occurred', ErrorCode.INTERNAL_SERVER_ERROR, null, requestId), 500);
};
