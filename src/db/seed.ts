 ...
 */
export async function assertSchemaPresent(): Promise<boolean> {
  ...
  if (!row?.present) {
    logger.error(
      '[Schema Missing] The "Projects" table was not found. ' +
 ...
 */
export async function pruneLegacyAccounts(editorUserId: number) {
  try {
    const stale = await query<{ UserId: number; Email: string }>(
      'SELECT "UserId", "Email" FROM "Users" WHERE "UserId" <> @editorUserId',
        ...
        for (const column of columns) {
          ...
          await execute(
            `UPDATE "${table}"
             ...
             WHERE "${column}" IS NOT NULL AND "${column}" <> @editorUserId`,
            [{ name: 'editorUserId', value: editorUserId }]
          )
        }
      }
...
export async function seedEditorAccount() {
  try {
    if (!(await assertSchemaPresent())) return
    ...
    const userId: number | undefined = upserted.recordset?.[0]?.UserId
...
export async function seedDemoProjects() {
  try {
    ...
    for (const p of projectsData) {
      const existing = await queryOne<{ ProjectId: number }>(
        'SELECT "ProjectId" FROM "Projects" WHERE "ProjectCode" = @code OR "Slug" = @slug',
        [
          { name: 'code', value: p.code },
      ...
      if (existing) {
        ...
        const locExisting = await queryOne(
          'SELECT "LocationId" FROM "ProjectLocations" WHERE "ProjectId" = @projectId',
      ...
      if (projectId) {
        ...
        await execute(
          `INSERT INTO "VRProjectSettings" ("ProjectId")
           VALUES (@projectId)
