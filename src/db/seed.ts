...
export async function seedRolesAndPermissions() {
  try {
    ...
    for (const [roleName, permCodes] of Object.entries(rolePermissionMappings)) {
      const role = await queryOne<{ RoleId: number }>('SELECT RoleId FROM Roles WHERE RoleName = @rName', [{ name: 'rName', value: roleName }])
      if (role) {
        for (const code of permCodes) {
          const perm = await queryOne<{ PermissionId: number }>('SELECT PermissionId FROM Permissions WHERE PermissionCode = @code', [{ name: 'code', value: code }])
          if (perm) {
