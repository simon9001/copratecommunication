...
export interface ServiceKeyInspection {
  valid: boolean
  role?: string
  projectRef?: string
  expiresAt?: string
 ...
 */
export function inspectServiceKey(key?: string): ServiceKeyInspection {
  if (!key) return { valid: false, problem: 'not set' }
  ...
  try {
    ...
    const projectRef: string | undefined = payload.ref
...
export function validateSupabaseConfigAtStartup(): void {
  ...
  if (!hasUrl && !hasKey) return // Not using the Supabase HTTP APIs. Fine.

  if (hasUrl !== hasKey) {
    logger.warn(
      `[Supabase] Only ${hasUrl ? 'SUPABASE_URL' : 'SUPABASE_SERVICE_ROLE_KEY'} is set. ` +
  ...
  if (dbRef && inspection.projectRef && dbRef !== inspection.projectRef) {
    logger.warn(
      `[Supabase] Project mismatch: DATABASE_URL points at project '${dbRef}' but ` +
  ...
  logger.info(
    `[Supabase] Service role key valid for project '${inspection.projectRef ?? 'unknown'}' ` +
