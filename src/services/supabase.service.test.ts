...
eq(
  ...
  inspectServiceKey('sbp_0102030405').problem,
  'not a JWT — expected three dot-separated segments'
)
eq('undecodable payload is rejected', inspectServiceKey('a.!!!not-base64!!!.c').valid, false)
eq(
  'an unexpected role is named',
  inspectServiceKey(fakeJwt({ role: 'authenticated', ref: 'r', exp: future })).problem,
  "unexpected role 'authenticated'"
)
...
eq(
  ...
  projectRefFromDatabaseUrl('postgresql://postgres:pw@db.abc123xyz.supabase.co:5432/postgres'),
  'abc123xyz'
)
eq(
  'transaction pooler puts the ref in the username',
  projectRefFromDatabaseUrl('postgresql://postgres.abc123xyz:pw@aws-0-eu-west-1.pooler.supabase.com:6543/postgres'),
  'abc123xyz'
)
eq(
  'a non-Supabase host yields no ref',
  projectRefFromDatabaseUrl('postgresql://user:pw@localhost:5432/kenha'),
  null
)
